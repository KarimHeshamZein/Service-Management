...
(function () {
  ...
  document.querySelectorAll("[data-store-movement-form]").forEach(function (form) {
    var lines = form.querySelector("[data-store-lines]");
    var add = form.querySelector("[data-store-add-line]");
    if (!lines || !add) return;
    function wire(row) {
      var remove = row.querySelector("[data-store-remove-line]");
      if (!remove) return;
      remove.addEventListener("click", function () {
        if (lines.querySelectorAll(".store-line").length > 1) row.remove();
        else row.querySelectorAll("input,select").forEach(function (field) { field.value = ""; });
      });
    }
    lines.querySelectorAll(".store-line").forEach(wire);
    add.addEventListener("click", function () {
      var row = lines.querySelector(".store-line").cloneNode(true);
...
// Keep professional settings accordions focused: opening one closes its siblings.
document.querySelectorAll("[data-exclusive-accordion]").forEach(function (accordion) {
  accordion.addEventListener("toggle", function (event) {
    ...
    if (!(opened instanceof HTMLDetailsElement) || !opened.open || opened.parentElement !== accordion) return;
    Array.prototype.forEach.call(accordion.children, function (sibling) {
      if (sibling instanceof HTMLDetailsElement && sibling !== opened) sibling.open = false;
    });
  }, true);
});

document.querySelectorAll("[data-user-access-form]").forEach(function (form) {
  function ensurePrimaryMembership() {
    var memberships = Array.prototype.filter.call(
      form.querySelectorAll("[data-membership-toggle]"),
      function (toggle) { return toggle.checked; }
    );
    var selectedPrimary = form.querySelector('input[name="primary_department_id"]:checked');
    var selectedMembership = selectedPrimary
      ? form.querySelector('[data-membership-toggle][value="' + selectedPrimary.value + '"]')
      : null;
    if (selectedMembership && selectedMembership.checked) return;
    if (memberships.length) {
      var replacement = form.querySelector(
        'input[name="primary_department_id"][value="' + memberships[0].value + '"]'
      );
      if (replacement) replacement.checked = true;
    }
  }

  form.querySelectorAll("[data-membership-toggle]").forEach(function (toggle) {
  function syncMembershipCard() {
    var card = toggle.closest("[data-department-access-card]");
    if (!card) return;
    var panel = card.querySelector("[data-membership-panel]");
    if (panel) {
      panel.querySelectorAll("input,select,textarea").forEach(function (control) {
        if (control !== toggle) control.disabled = !toggle.checked;
      });
    }
    if (!toggle.checked) card.open = false;
    ensurePrimaryMembership();
  }
  toggle.addEventListener("change", syncMembershipCard);
  syncMembershipCard();
  });
  form.addEventListener("submit", ensurePrimaryMembership);
});
