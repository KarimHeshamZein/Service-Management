...
person and contact number. The existing `sites` table remains the compatibility
storage layer; for new and edited projects its legacy `customer_name` value is
kept equal to the project name.
...
188 tests across the PostgreSQL regression suite:
...
Every new Installation, Preventive Maintenance and Maintenance record requires
an existing quotation ID belonging to the selected Project. All record
submitters may select the identifier without seeing prices. Only Administrators
