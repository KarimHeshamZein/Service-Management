...
def submit_form(
    ...
    if append_to is not None:
        ...
        form = {
            "participants": selected_ids_for_names(choices, record.participant_names),
            "devices": [{}],
...
async def submit_record(
    ...
    if append_record_id_raw:
        ...
        if append_record_id is None:
            errors["form"] = "The preventive maintenance record being edited is invalid."
        else:
            ...
            "devices": [
                {
                    ...
                    "photo_guidance_profile_id": entry["photo_guidance_profile_id"],
                }
    ...
    if append_record is not None:
        ...
        appended_report_numbers = delete_linked_reports(
            db, ServiceReportRecord.preventive_record_id, append_record.id
        )
        add_revision(
            db,
    ...
    if append_record is not None:
        message = f"Sites and services added to preventive maintenance record {record.record_number}."
...
def records_list(
    ...
    if not user.is_admin:
        conditions.append(or_(
            ...
            MaintenanceRecord.site_id.in_(user.assigned_project_ids),
        ))
...
async def edit_record(
    ...
    scope_map = {
        ...
        for scope in grouped_edit_scopes(record, all_wrappers)
    }
    valid_scope_positions = {
        int(getattr(wrapper["data"], "scope_position", 0) or 0)
        for wrapper in wrappers
    }
    saved_rows, table_error = parse_saved_table_rows(
        form, valid_scope_positions, installation=False
    )
    if table_error:
        return render(
            request,
            "record_edit.html",
            _edit_context(request, db, record, table_error, selected_participant_ids),
            status_code=422,
        )
    ...
    if removed_wrappers:
        changes["removed_services"] = [
            {
                "item_id": wrapper["data"].id,
                "site": wrapper["data"].work_site_name or record.site_name,
                "service": wrapper["data"].service_name,
            }
        ...
        for wrapper in removed_wrappers:
            ...
            if wrapper["data"] in record.work_items:
    ...
    if removed_wrappers:
        sync_legacy_photo_mirror(record, MaintenancePhoto)
    if record.work_items:
        ...
        replacement_rows.append(
            MaintenanceDataRow(
                **row,
                ...
                sub_project_id=scope["sub_project_id"],
                sub_project_name=scope["sub_project_name"],
                work_site_id=scope["work_site_id"],
                work_site_name=scope["work_site_name"],
            )
        )
    ...
    # A newly added Site/service row is an attempted addition even when the
    # technician forgot to choose the Service.  Treating only a selected
    # service as intent incorrectly redirected with "No changes were made" and
    # discarded the visible validation context.
    ...
    if not changes and not has_additions:
        ...
        if _is_ajax(request):
            return localized_json(
                request,
                {"ok": True, "redirect": f"/maintenance/records/{record.id}"},
            )
    ...
    if has_additions:
        ...
        if response.status_code < 400:
            ...
            set_audit_context(
                request,
                action="update",
                entity_type="preventive_maintenance_record",
                entity_id=record.id,
                entity_label=record.record_number,
                changes={**changes, **appended_changes},
            )
        else:
            db.rollback()
            delete_stored(
